...
This guide covers the use of Blender starting with version 2.8, including {version} or higher for creating content for Open Rails. It is recommended to use the latest Long Term Support (LTS) release of Blender for the best compatibility with the instructions provided in this document. Please note that older versions of Blender will not be covered or supported in this guide. At the time of this update, Blender Version {BV} is the most recent release and {version} is the most recent Long Term Support release. 
