...
The next step is choose the VECTOR _TAB_, with the TEXT selected, chose "convert to curves" in the pulldown menu.

image::images/Screenshot 2026-04-08 130033.png[]


And finally, chose the outline thickness, here I chose a value of 7.8.  And its done.
...
PBR Materials:: - Physically Based Rendering (PBR) is a dynamically generated lighting method that replaces the 'baked' lighting methods of {msts} that are based on a single texture application on a model.  It uses multiple images to emulate lighting including reflections and depth for a more accurate visual appearance.

glTF:: - This is a newly introduced model source format which {OR} understands that utilizes PBR materials.  Note: The `.S` file format is not going away. 

[TIP]
Baking ambient occlusion layers require objects to occupy unique texture real estate - if you want to apply details or baked textures - you need to ensure no UV Island overlaps another. If you have used the "project from view" option while UV Mapping, AO Baking will be troublesome unless care is taken to deal with overlaps and hidden faces.  Baking also has an option to adjust the margin size and its probably best to keep the margin to a minimum.
...
[TIP]
Remember that having fewer texturs files for your model reduces _draw calls_ at run time. A larger texture uses more video card memory, but modern cards are designed for this.
...
Your first step is to generate the Base Color texture.  When using {ms}ts or traditional {OR} models, this is actually where you stop in texture generation.  With PBR textures, its just the beginning.  Base Color is the name of the input used on the "Principled BSDF" shader node for this type of image. 

A metallic map is next in the list and is used to simulate a metal surface, which often react with light in a specific way.  The Metallic Map is often a binary color image that is made of just black and white pixels (0 and 255) color values. The metallic setting in the shader can be adjusted from 0 to 1. Color Space is non-color for this image.

The Roughness Map shows how rough or smooth the surface is.  This affects the sharpness of reflected light, in essence, how much light us scattered by the surface to produce gloss or matte surfaces. The Roughness setting in the shader can be adjusted from 0 to 1. (0 = gloss 1 = matte) Color Space is non-color for this image

The next step in PBR texture generation is generating the Normal Map.  You might recall from other sections in this document that a Normal is related to the Facing Direction of a 3D surface. When ever light hits this specific surface, the normal setting would be able to allow for the light reflection of the "outside" surface.  The NORMAL is also a determination of the inside and outside facing direction of a surface.  An improperly facing normal will cause incorrect lighting of a surface to occur.

So, how does the Normal PBR texture work?   The Normal Map is used to develop a sense of depth in a surface or in other words, the bumpiness. The RGB values in the image translate to XYZ coordinates without changing the underlying geometry. A normal map can used where extra geometry in the underlying model would be costly. (Think: Doors on a the long hood of a diesel engine)   Normal Maps often look very purple, as a side effect of using the RGB color space for x,y,z displacement data.  Color Space is non-color for this image. 
