...
About ORTS:: The Open Rails Train Simulator is an open source project that exists primarily because the promised Microsoft Train Simulator 2.0 never happened. {OR} attempts to be backwardly compatible with nearly all existing {MSTS} content while adding many new features and corrects old issues that existed in the Microsoft/Kuju releases.  Very often, you can use MSTS and ORTS terms interchangeably, but keep in mind that ORTS can handle higher poly count (more complex) models than {MSTS} can as well as having many other changes added that are unique to {OR}. 

(((Open Rails, "The Open Rails Train Simulator, free and open source project" )))

OPEN RAILS::
The key advantage that Open Rails currently offers over Microsoft Train Simulator is that good frame rates can be maintained with a much higher number of polygons, so curves can be smoother and more detail can be modelled. Open Rails also displays 32-bit color (whereas Microsoft Train Simulator is limited to 16-bit). Another advantage is the longer viewing distances, adjustable from 2km out to 10km.
...
HDRI	::	 HDRI is short for High Dynamic Range Image. Digital cameras only have a limited dynamic range â€” that's why some areas of a photo appear darker than they do in real life. HDRIs give photo editors a chance to brighten the corners and create an image that looks more natural.  Environment maps or HDRI maps are one of the most efficient and quickest ways to light your 3D scene and achieve realistic results in Blender. HDRIs are essentially a snapshot of the real-world lighting which contains accurate lighting detail through high dynamic range imaging (HDRI)
HIERARCHY	::	 The assignment of parenting of objects within a shape so that an object's children will only move when they do but they can move independently too but are always affected by their parent.
...
MANIFOLD SHAPE	::	   A manifold shape is a 3D object that is considered to be watertight.  The vertices, faces and edges are aligned and connected in such a way that if you poured water into the INSIDE of the object, none would leak out.  A default `cube` primitive is an example of a manifold shape.
...
.Author's Advice
----
Start with the option that is easiest for you to understand and then adjust as you go. Some content developers are well-versed in Blender and can help you with the modeling, but keeping it simple will help you get started faster.  Maybe just start with a `Made for Open Rails` only project, for example.
----
...
Required::
* Blender {LTSversion} is the most current release. Blender 4.5 LTS can also be used for maximum add-on compatibility because Blender 5 is relatively new.
...
[NOTE]
Using METRIC for measurements. It's the default measurement in Blender and {OR}.  It is realized that some people have become very attached to their _U.S. customary units_   but Metric does make for worldwide compatibility and in the end it just ends up being easier if you stick with it. 
...
Non-Destructive Workflow:: Blender supports a non-destructive modeling techniques through modifiers and procedural workflows, allowing you to make changes to your models without losing the original data.
