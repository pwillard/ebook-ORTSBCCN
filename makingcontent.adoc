...
* A 3D program, {version} is recommended, but even the LATEST release - currently {BV} - should also be fine unless the *Blender's PYTHON API* has dramatically changed. (In the case of Blender 5.0, this is the case and the Exporter add-on will need to be updated.) 
...
* 3d Modeler: {bv}
* Bitmap tool (any one of these, interchangeably): Canva, Affinity Studio (free), or Paint.net
* UV/Shader tool: Blender has this built in; 3rd party options exist (like Substance Painter ), however I don't use them.
* Text Editor: _Microsoft Visual Studio Code_ 
* Metric Conversion Calculator (I have one built into a hand-held calculator, but you can use Google for this as well)
...
To export your model for use with {MSTS} or {OR}, use Wayne Campbell's Blender MSTS/Open Rails exporter add-on for {MSTS}-format `.s` files.


=== What I Can Recommend:

* 3D Modeler: Blender {LTSversion} or the current stable {BV} release is recommended. Blender 3.6 LTS can still be useful for older workflows, but Blender 2.79-era workflows are no longer covered here. Did I mention that Blender is free?
* Bitmap tools: _Affinity Photo_, _Paint.NET_ (free), _GIMP_, _Krita_, or _PaintShop Pro_ if you already have it.
* UV tool: Blender's built-in UV and shader tools are enough for the workflows in this notebook. Third-party tools can help, but they are optional.
* Text Editor: _Microsoft Visual Studio Code_ , _Context.exe_ or _Sublime_ 

.Author's Note:
****
Much of this document was created with Blender 3.6 LTS and updated while using Blender {BV}.
****

[NOTE] 
CONTEXT is an abandoned editor, but it had a useful syntax highlighter for ENG and WAG configuration files. For current work, Visual Studio Code is the safer recommendation; an ENG/WAG syntax-highlighting project is available at https://github.com/pwillard/engwag
...
[NOTE]
According to Blender.org, new Blender versions are released regularly. This notebook assumes a 64-bit Windows installation. The safest choice is the current `LTS` release, while the latest stable version is also suitable if the exporter add-on supports it. This document is currently being checked against Blender {BV}.
...
`C:\Users\willard\AppData\Roaming\Blender Foundation\Blender\4.5\config`   
...
The following section applies to Blender version 3.0 and later. Be aware that certain changes introduced in Blender 4.0 or Blender 5.0 may affect the procedures described here, particularly those related to add-on installation.

Beginning with Blender 4.0, many built-in add-ons are no longer bundled directly with the installer. Instead, they are downloaded and installed over the network after the initial setup. This change may alter the steps required for managing or enabling add-ons in newer versions.
...
[NOTE]
Blender 4.x now includes a built-in Add-On Manager that allows you to install add-ons from the internet.  This is the preferred method of installing add-ons.  This is the method we will use to install the add-ons we need for {OR} when using the latest version of Blender.  There is also a new option called `Extensions` for add-ons that meet Blender's community standards. _You must have enabled the "Allow Online Access" from the network settings under "System" preferences for this feature to work._ 

[TIP]
With the latest versions of Blender, to install an add-on, click the "Install" button on the right side of the add-on you want to install.  This will download the add-on and install it in the correct location for Blender to find it.  You can also search for add-ons by name or category using the search bar at the top of the Add-ons window.


===== Installing Add-Ons with Version 4.x
...
|Verify it supports Blender 4.3
...
==== Also in Blender 4.x - Extensions

image::images/editors_preferences_section_extensions.png[align=center]

Here’s a detailed breakdown of the Extensions system introduced in Blender (starting around version 4.x), how it differs from the traditional Add-ons system, and how you can use it in your workflow.
...
[NOTE]
While “Add-ons” is still a valid term, in newer versions the UI uses “Extensions” to highlight the full range of extension types, not just Python scripts. 
...
[NOTE]
Blender has discontinued including all of the built in add-on that it previously included in the installation distribution. You can still get them, you just need to install them from the internet now.
...
* Instructions say to unzip the file before using the Blender `install` option, as it won't install correctly due to the additional support files included with the distributed `.zip` file.
...
* Price: $5.00
* This add-on allows you to export your selected model object to a new `.blend` file.
