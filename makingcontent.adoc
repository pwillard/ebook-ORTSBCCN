...
Made for MSTS::	Meaning it *can* run in ORTS, but it might need some editing to the WAG or ENG file since some Physics values from lagacy {msts} might cause the vehicle to not perform optimally
...
Made for ORTS - but requires other add-ons::  Meaning it *can* only run in ORTS as it is configured specifically with {or} configurations and parameters and it will need additional files downloaded to work
...
[NOTE]
I recommend just using METRIC for measurements.  I realize that some people have become very attached to their imperial units but Metric does make for worldwide compatibility and in the end it just ends up being easier. 
