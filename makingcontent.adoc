...
* Metric Conversion Calculator (I have one built into hand held calculator, but you can use Google for this as well)
...
To export your model for using with {MSTS} or {OR}, there is a very capable exporter add-on for {MSTS} format `.s` files.
...
* BitMap Tools: _Canva Affinity Studio_ (free), _Paint.net_ (free), _PaintShop Pro_ (even version 7 still works), or download the latest version of _GIMP_ or _Krita_ for free.
...
.Author's Note:
****
Much of this document was created with Blender version 3.6 LTS and but I have been using {BV} lately.
****

[NOTE] 
CONTEXT is an abandoned editor, but it has a nice syntax highlighter for ENG and WAG configuration files available at github where this document is maintained. There is an early version of a context highlighter in the works for ENG/WAG files in VScode, but it's not finished it yet.  https://github.com/pwillard/engwag
...
[NOTE]
According to the Blender.org website, a new version is released about once per quarter.  The assumption is that you are installing Blender on a Windows 64 BIT platform.  It is recommended that you download the `LTS` or Long Term Support version and if you are really brave, you can download the latest available stable version. {BV} is currently being used, a new release is available.
...
The use of `Include()` statements in SIMIS files is becoming more common in {OR} configurations but use if the INCLUDE feature us entirely optional.
...
This is particularly beneficial when you are create a group of similar cars that may only differ in a few details or maybe just the car reporting marks.
...
* It is probably quite helpfule to liberally use "COMMENT()" sections in your INC file
* INC files are only really useful for configration items that are shared among multiple locomotives or cars. 
...
==== Specifiying relative references


(((Configuration, "relative reference")))

Getting {OR} to locate your INC files is important.  You will use relative referenes to refer to include files by including in a way that references their location to the main file.  Here is an example of how to do this:
...
Within this file you can see all of the key sections of the configuration file.  The first section is the Wagon definition, which contains information about the wagon itself, such as the type, shape, name, size, and mass.  The second section is the Coupling definition, which contains information about the coupling between the wagon and the train, such as the type of coupling, the stiffness, damping, break force, and velocity.  The third section is the Derail and  and Braking definition, which contains information about the brake system, such as the type of brake equipment, the type of brake system, and the brake cylinder pressure for the maximum brake force and Davis Coefficients.  The final section refers to the sound file used by the wagon.

You will notice that the S (shape definitione) and the SOUND entry are references to external files.  These are references that are compatible with legacy MSTS. 
