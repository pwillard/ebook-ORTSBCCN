...
Made for ORTS - but requires other add-ons::  Meaning it *can* only run in ORTS as it is configured specifically with {or} configurations and parameters in mind but it will need additional files downloaded to work
...
Made for ORTS but requires MSTS and other add-ons:: Meaning it *can* only run in ORTS as it is configured specifically with {or} configurations and parameters but also requires {msts} to be installed and relies on additional products. 
...
* An Exporter add-on for the MSTS/ORTS format, we will use Wayne Campbell's _S File Exporter_ from Elvas Tower web sitefootnote:[Download from https://github.com/pwillard/Blender_MSTS_ORTS_Exporter]
...
* 3D Modeler: Blender 2.93 LTS version or {version} or even {BV} should be fine. (If you are already well accustomed to Blender 2.79 it's OK, but you will be on your own here) Did I mention that it is free?
...
| Blender28toMSTS |Available From: https://github.com/pwillard/Blender_MSTS_ORTS_Exporter/blob/main/Blender_MSTS_ORTS_Exporter.zip See the video:footnote:[Youtube https://youtu.be/j3AVw7s9qoA] Instructions say to un-zip the the file before using the Blender `install` option, as it won't install correctly due to the additional support files included with the distibuted `.zip` file. DOCS: https://github.com/pwillard/Ebook-MSTSORTSExporter/blob/main/MSTSORTSExporter.pdf
...
[TIP]
Reminder, this is basically a "notebook" and started life as my collection of notes about content creation.  It may seem to jump around a lot as a result.
...
On of my mentors while I was in my beginning stages of learning Blender was Josh and Ryuu of Blender Brothers.  I subscribed to their Youtube channel and I get regular email updates about their ideas and helpful tips.
