...
About ORTS:: The Open Rails Train Simulator is an open source project that exists primarily because the promised Microsoft Train Simulator 2.0 never happened. {OR} remains compatible with most existing {MSTS} content while adding new features and correcting long-standing Microsoft/Kuju limitations. In many places you will see MSTS and ORTS terms used interchangeably, but keep in mind that {OR} can handle more complex models, 32-bit textures, longer viewing distances, and simulator features that never existed in {MSTS}.

(((Open Rails, "The Open Rails Train Simulator, free and open source project" )))

OPEN RAILS::
Open Rails can maintain good frame rates with a higher number of polygons than {MSTS}, so curves can be smoother and details can be modeled directly when it makes sense. Open Rails also displays 32-bit color, while {MSTS} is limited to 16-bit color, and it supports longer viewing distances that can be adjusted from about 2 km out to 10 km.

_from Open Rails.org website_

Since this document is primarily about using Blender, other 3D modeling tools are mentioned only where they provide useful contrast or conversion context.

<<<

== Common Terms and Definitions
(((Making Content, Terms and Definitions)))

There is just no way to avoid using the jargon related to 3D modeling and content creation. Here are some common terms and definitions that you should become familiar with:

ALPHA	::	 A bitmap with a portion of the image masked out, making that area appear transparent in the simulator.
DISTANCE LEVELS	::	 See Level of Detail (LOD)
EDGE	::	 A line segment connecting two vertices.
ENG FILE	::	 A file that describes the simulator details for a locomotive or powered unit. It contains values for traction, braking, couplers, lights, sounds, and other behavior.
FACE	::	 A surface created by three or more vertices.
...
HARD SURFACE MODELING	::	 A generalized term for modeling objects such as vehicles, buildings, roads, tools, and machinery. Hard-surface work usually relies on standard mesh techniques and primitives such as cubes, cylinders, planes, cones, and torus objects.
HDRI	::	 HDRI is short for High Dynamic Range Image. Environment maps or HDRI maps are one of the quickest ways to light a 3D scene and achieve natural-looking results in Blender because they capture real-world lighting across a wide brightness range.
HIERARCHY	::	 The parenting relationship between objects in a shape. Child objects inherit movement from their parent but can still have their own local transforms.
LEVEL OF DETAIL	::	 (LOD) Distance values at which the base model is replaced by reduced-poly versions because fine detail cannot be seen at long range. Higher-distance LOD models should usually contain less detail. There are no universal LOD ranges, but you should consider using them for complex models.
MANIFOLD SHAPE	::	 A manifold shape is a 3D object that is considered watertight. Its vertices, faces, and edges are connected so that if the object were filled with water, none would leak out. A default `cube` primitive is an example of a manifold shape.
MATERIAL	::	 A material is assigned a texture with additional components such as lighting palette and shading.		
MESH	::	 A 3D model made from vertices, edges, and faces. Textures and materials are applied to the mesh after the geometry exists.
NGON	::	 A face with five or more vertices. Many Blender tools can work with n-gons during modeling, but exported real-time content often behaves best when final faces are kept simple and predictable.
NORMAL	::	 A normal is a direction vector perpendicular to a face. Normals control which way a face points and affect lighting, culling, and texture display.footnote:[See https://en.wikipedia.org/wiki/Normal_(geometry)]
...
POLY COUNT	::	 The total number of polygons used to create a 3D mesh. It is one rough indicator of how complex a model is.
POLYGON	::	 A planar face in a mesh. Its edges define its boundary, and its vertices are the points where those edges meet. Blender commonly refers to triangular faces as `tris` and four-sided faces as `quads`.
QUAD	::	 A face made from four vertices.
SHAPE	::	 A finished three-dimensional object ready to be placed within the simulator environment.
SMOOTHING	::	 A shading method that controls how light appears across individual polygons or groups of polygons.
TRI	::	 A face made from three vertices, also called a triangle.
UV	::	 The process of projecting a 2D image onto a 3D model's surface for texture mapping. The letters `U` and `V` are used for the 2D texture axes because `X`, `Y`, and `Z` are already used for 3D object space.
VERTEX	::	 A point in 3D space with X, Y, and Z coordinates. The plural is vertices. Edges and faces are built from vertices.
WAG FILE	::	 A file that describes the simulator details for a wagon or unpowered item of rolling stock. It contains values for braking, couplers, lights, sounds, and other behavior.
...
Made for {MSTS}::	Meaning it can run in {MSTS} and usually in {OR}, but it may need WAG or ENG edits because some legacy physics values do not perform as well in Open Rails.
Made for {OR} - no other files required::	Meaning it is configured specifically for {OR} and does not depend on outside route, stock, or texture packages.
Made for {OR} - requires other add-ons:: Meaning it is configured for {OR}, but it requires additional downloads to work correctly. NAVS-based items are a common example.
Made for {OR} but compatible with {MSTS}:: Meaning it runs in {OR} but has also been configured to remain compatible with {MSTS}. This usually takes more work than targeting {OR} only.
Made for {OR} but requires {MSTS}:: Meaning it runs in {OR}, remains compatible with {MSTS}, and also requires files that only come from a {MSTS} installation.
Made for {OR} but requires {MSTS} and other add-ons:: Meaning it is configured for {OR}, but it also requires {MSTS} files and additional third-party content.

Deciding on which option to use might be difficult, but you should try to make a decision based on the following criteria:

* How comfortable are you with Open Rails and its content options?
* How much effort do you want to put into this project?
* What is your experience with 3D modeling in general (Train Sim Modeler, GMAX, 3D Canvas, etc.)?
* What is your experience with 3D modeling in Blender?
* What is your experience with 3D modeling for Open Rails compared to {MSTS}?
* How much flexibility do you want to have with your content?

.Author's Advice
----
Start with the option that is easiest for you to understand, then adjust as you go. Some content developers are well-versed in Blender and can help with modeling questions, but keeping the first project simple will help you finish it. A `Made for Open Rails` project with no outside dependencies is a good starting point.
----

[TIP]
Share your work. Don't work in a vacuum. Talk to other content developers and see what they are doing. This will help you get feedback and help you avoid making the same mistakes as others.  It also creates a level of involvement and accountability that can help you grow as a content creator and give you a push to not leave projects half-done.


== Content Generation

To go over some of the steps involved in creating content for Open Rails, we will create a quick list of the steps involved in creating a simple train related model.  This is not a comprehensive list of all the steps involved, but it should give you a good idea of what is involved in creating content for Open Rails.

(((Blender, "Getting Started")))
 
* A mesh, also known as your 3D model.
* Online resources and references. This area is still somewhat limited for {OR}-specific Blender work, but useful community references include https://www.trainsim.com and http://www.elvastower.com/forums/index.php.
* A texture: a two-dimensional bitmap image that you apply to your model, also referred to as a skin.
* A UV map for the mesh: the instructions for mapping your 2D texture to your 3D model. This is usually best done inside Blender, although dedicated UV tools can also be used.
* A configuration file or set of files that describes your content to the simulator. For rolling stock this usually means a WAG or ENG file.
* A thumbnail image, usually JPG or PNG. This is optional but helpful.
* Instructions. Do not leave the work half done.

=== Recommended Software and Tools

Required::
* {LTSversion} is the most current release. Blender 4.5 LTS can also be used for maximum add-on compatibility because Blender 5 is relatively new.
* The {MSTS}/{OR} exporter add-on maintained by PWILLARD on GitHub.footnote:[Download from https://github.com/pwillard/Blender_{MSTS}_ORTS_Exporter]
* A bitmap editor with layers and PNG export, such as Affinity Studio, Paint.NET, GIMP, or Krita.
* A text editor that handles Unicode files. _Microsoft Visual Studio Code_ is recommended.

Recommended::
* Blender's built-in UV and shader tools are enough for the workflows in this notebook. Third-party tools can help, but they are optional.
* A metric conversion calculator. Google or a hand-held calculator is sufficient.
* A scale calculator for converting dimensions from a scale drawing. There is one at http://www.railsimstuff.com.
* A texture snipping or correction tool. Windows has a built-in Snipping Tool, and TextureRipper is a modern option for perspective-correcting texture photos: https://github.com/raycastly/texture-ripper/releases. SHOEBOX can still be useful, but it depends on the abandoned Adobe AIR runtime.

Optional::
* If you already use _Context.exe_ or _Sublime_ for text editing, they can still be useful. CONTEXT is abandoned, but it had a useful syntax highlighter for ENG and WAG configuration files. For current work, Visual Studio Code is the safer recommendation; an ENG/WAG syntax-highlighting project is available at https://github.com/pwillard/engwag.
* If you are using _Visual Studio Code_, I recommend the _ORTSFileSupportForVSC_ syntax highlighter from `matejpacha` on GitHub: https://github.com/matejpacha/ORTSFileSupportForVSC/releases.
* If you are also making models for Trainz Simulator, note that the latest versions of Trainz will accept Blender's native FBX exporter output.

[NOTE]
Use metric measurements. Metric is Blender's natural working assumption for this notebook and is the safest choice for {OR} content. Even if your prototype dimensions come from U.S. customary drawings, convert them to meters early and keep the Blender scene in metric units.

=== Why the Author Uses Blender 

(((Blender, "Why Blender?")))

From Pete Willard:: I started creating game content in 2002, primarily for {MSTS}. I have used Abacus Train Sim Modeler, 3D Canvas, Gmax, SketchUp, and Blender. 3D Canvas was my favorite for many years, but I now use it mainly as a file-conversion tool. Blender {BV} has the tools I need for 3D modeling, plus active development and support.

== Why Do Many People Use Blender to Make Content?
...
Open-Source and Free:: Blender is open-source software, which means it's free to use and has an active community of developers and users. This makes it accessible to a wide range of users, including hobbyists, indie game developers, and professionals.

Versatility:: Blender offers a comprehensive set of tools for 3D modeling, sculpting, texturing, rigging, animation, rendering, and more. It can handle a variety of asset types, from characters and environments to props and special effects, making it a versatile choice for game development.

Integrated Workflow:: Blender provides an integrated workflow, allowing you to create, edit, and animate assets seamlessly within a single application. This can save time and reduce the need to switch between different software tools.

Asset Optimization:: Blender provides tools to help optimize your assets for real-time rendering in games. You can create LODs (Level of Detail), bake textures, and manage UV maps to ensure your assets are efficiently displayed in real-time environments.

Community and Resources:: Blender has a large and active community of users, artists, and developers. This means there are plenty of tutorials, forums, and resources available to help you learn and improve your skills.

Non-Destructive Workflow:: Blender supports non-destructive modeling techniques through modifiers and procedural workflows, allowing you to make changes to your models without losing the original data.

Constant Development:: Blender's development is ongoing, with new features and improvements being added regularly. This ensures that the software stays up-to-date with industry trends and user needs.  In fact, Blender might get updates faster than you can keep up.
...
To export your model for use with {MSTS} or {OR}, use the PWILLARD-maintained Blender MSTS/Open Rails exporter add-on for {MSTS}-format `.s` files.

.Author's Note:
****
Much of this document was created with Blender {older} and updated while using Blender {BV}.
****
...
[NOTE]
Blender versions are released regularly. This notebook assumes a 64-bit Windows installation. The safest choice is the current `LTS` release, while the latest stable version is also suitable if the exporter add-on supports it. This document is currently being checked against Blender {BV}.
...
[WARNING]
If you are still using Windows 7, current Blender versions will not install. You should upgrade Windows before relying on this notebook. Some concepts still apply to older Blender versions, but the current workflow assumes a supported Blender release.
...
[NOTE]
Between version updates, Blender developers may move some interface features. You may not see the exact same screens shown in this document.

Let's use the Blender installer from the *Blender.org* website. https://Blender.org/download/

The LTS version is linked from Blender's download page under "Looking for Long-Term Support?" It can also be found here: https://www.blender.org/download/lts
...
According to Windows POWERSHELL, an example of the %APPDATA% Environment variable points to: 
...
This means that Blender *USER DATA* such as `User Preferences`, default `Startup` blend files, and third-party add-ons will be stored in the `Blender Foundation` folder under

`C:\Users\willard\AppData\Roaming\` + 

in other words, my config files and addons are located at + 

`C:\Users\willard\AppData\Roaming\Blender Foundation\Blender\5.1\config`
...
Much of this section is optional, but it will help you get started.  Also note: these settings and steps may vary depending on your version of Blender.
...
`%appdata%\Blender Foundation\Blender\5.2\config\userpref.blend`
...
Screen with options changed to what _Pete Willard_ prefers...

<<<

==== Themes-3D Viewport

image::images/PREFS3.PNG[align=center]
...
[TIP]
If using Blender 4.3 or newer, you should also go into "System Settings" under Network and check "Allow Online Access".
...
The following section applies to Blender 3.0 and later. Changes introduced in Blender 4.2 and Blender 5 may affect some procedures, especially add-on installation.

Beginning with Blender 4.2, many add-ons are managed as extensions and may be downloaded after installation instead of being bundled directly with Blender. This changes how some add-ons are found, installed, and enabled in newer versions.
...
=== Customizing Asset Library Locations

In this setup, the default *user* library location has been renamed to *FREIGHT*, and its folder path has been updated accordingly. An additional library folder has also been added for *ENGINE*-related assets. 

These changes are purely a matter of personal preference—library folders can be named and organized in any way that suits your workflow. Additional folders may be added at any time by clicking the "+" icon located at the bottom right of the *Asset Libraries* window.
...
[NOTE]
Blender 4.2 and newer include the Extensions system for finding and installing add-ons, themes, and other packages from Blender's online repository. This is the preferred method when an add-on is available there. _You must enable "Allow Online Access" in the Network settings under *System* preferences for this feature to work._

[TIP]
In current Blender versions, search for extensions by name or category, then click *Install* beside the item you want. Blender downloads and installs it in the correct user location.


===== Installing Add-Ons in Blender 4.2 and Newer

Blender add-ons expand Blender’s functionality with new tools,
import/export features, UI panels, and automation utilities. Add-ons
may be included with Blender or provided as third-party ZIP packages.

===== Enabling Built-In Add-ons

. Open *Edit → Preferences…*
. Select the *Add-ons* tab
. Use the search box to locate the desired add-on
. Enable it by checking the box next to its name

NOTE: Some bundled add-ons may require a small download the first time
they are enabled.

===== Installing Third-Party Add-ons

. Download the add-on package in `.zip` format
. In Blender: *Edit → Preferences → Add-ons*
. Click the *Install…* button in the upper-right
. Select the ZIP file and click *Install Add-on*
. Enable the add-on using its checkbox

TIP: Do not extract the ZIP file unless you are specifically instructed to.
The ZIP must contain an `__init__.py` file in its root folder to install properly.

==== After Installation

Some add-ons:

* Add new panels to the Sidebar (press `N`) in the 3D Viewport
* Add menu items under *File → Import* or *File → Export*
* Provide configuration options under the add-on’s *Preferences* section

If *Auto Save Preferences* is enabled (it is by default), Blender
will remember your enabled add-ons next time it starts.

=== Add-on File Locations

|===
|Type |Location

|Built-in Add-ons
|Installed with Blender in the program directory

|User-installed Add-ons
|Stored in the user profile without requiring administrator access
|===

To reveal the installed location:
*Preferences → Add-ons →* Gear icon → *Show in File Browser*
...
|Ensure you selected a ZIP containing an `__init__.py` (not a folder inside another folder)

|Add-on throws errors
|Verify it supports your Blender version, especially if you are using Blender 5

|Panel or tool not visible
|Check the Sidebar (`N`) or menus under *File → Import/Export*

|Add-on disabled after restart
|Enable *Auto Save Preferences*
|===

TIP: Download add-ons from trusted sources (GitHub, Blender Market, etc.).

==== Extensions in Blender 4.2 and Newer

image::images/editors_preferences_section_extensions.png[align=center]

The Extensions system, introduced in Blender 4.2, provides an in-Blender way to find, install, update, enable, and remove approved packages.

*What are “Extensions”?*

In Blender, “Extensions” is a term encompassing add-ons, themes, keymap packages, and other modular enhancements to the core software. 


The official online platform for these is Blender Extensions Platform (via extensions.blender.org). It is a repository and sharing site for free/open-source extensions. 

The key idea: make it easier to find, install, enable, update, and manage these modular enhancements all from within Blender. 


[NOTE]
While “add-on” is still valid, newer Blender versions use “Extensions” for the broader package system, not just Python scripts.

Enabling the feature: (first time only) +

* Open Edit → Preferences → go to Extensions tab (or sometimes Get Extensions).

* If prompted, allow Online Access so you can see the repository of extensions. 

* Once enabled, you'll see the list of available extensions from the official platform inside Blender.

Browsing for new extensions:

* Click on the “Get More Extensions” button at the bottom of the Extensions tab.

* This will open the Extensions Platform website in your default web browser.

* You can search for extensions by name, category, or author.

Installing extensions:

* Click on the “Install” button next to the extension you want to install.

* This will download and install the extension in the correct location for Blender to find it.

Enabling and disabling extensions:

* Click on the checkbox next to the extension you want to enable or disable.

* The extension will immediately be enabled or disabled.

Updating extensions:

* Click on the “Update” button next to the extension you want to update.

* This will check for updates and install them if available.

Uninstalling extensions:

* Click on the “Uninstall” button next to the extension you want to uninstall.

* This will remove the extension from Blender.

*How to use Extensions in Blender*

Extensions are modular enhancements to the core software that can be enabled and disabled as needed. 


==== Enabling and Disabling Add-Ons

[NOTE]
Some add-ons that used to be bundled with Blender are now installed through Extensions. They are still available, but you may need to download them from inside Blender.
...
To get us started, here are a few Add-ons I recommend installing:
...
1) Blender MSTS/Open Rails Exporter
* Available from the GitHub releases page: https://github.com/pwillard/Blender_MSTS_ORTS_Exporter/releases
* Download the installable add-on ZIP from the latest release and install it from Blender's add-on installer.
...
* A Python script to create curve shapes from edges that can be converted back to a mesh. It can be used to create handrails and pipes.
* This is still valid, but its capabilities have largely been superseded by the Blender Extension named *ND* from HugeMenace.

3) TrainSimTools
* Available from: https://github.com/pwillard/TrainSimTools/releases/tag/v1.0.0
* This is an optional add-on that is available to assist with setup and manipulation of the modeling process specific to MSTS/ORTS modeling.
* For example, it will create the required "collections" setup on-the-fly so you don't have to hard-code the collection structure into your startup configuration.  This is helpful when you are working with multiple models where some of them are not Train Sime related. 
...
There are hundreds of add-ons that are not distributed with Blender and are developed by others. To add them to the list of available add-ons, they must be installed into Blender. BlenderMarket (Now called superhive, for some reason : https://superhivemarket.com/) and GUMROAD are the two most popular places to find add-ons.
...
These steps help to make things more visible while editing objects.

(((Blender, "MATCAP")))

[TIP]
I also sometimes choose MATCAP and pick the leftmost MATCAP option for better visual representation of what I'm working on in the early stages of modeling. It can help to see the shape of an object in a more 3D way and also makes surface errors easier to see.
...
(((Blender, "Units, Feet, Meters, Metric, U.S. customary units ")))

If you need to work in U.S. customary units  versus Metric, you would adjust the setting under the UNITS SYSTEM option in Scene Properties.  

[IMPORTANT]
Changes to `UNIT` settings can have effects in other areas, such as Camera View settings related to  `Focal Length`, `Clip Start` and `Clip End`. To adjust these values after changing UNITS settings, use the _N-Panel_ `View` tab to make adjustments.  If you end up with unusual clipping or part of your model disappearing when you zoon out, you need to adjust your _clip end_ settings.
...
This step requires that you have already downloaded an example HDRI file from one of the HDRI WEB SITE locations mentioned at the beginning of this document.  The downloaded HDRI files could be located in your menu:Documents[Blender > HDRI] folder that you create for this purpose.  See TIPS below for usage details.

In the "World Properties" panel (globe icon on the right),  Add "Background" in the Surface pulldown menu.  In the Color section, select "Environment Texture". This will allow us to choose the HDRI we downloaded and made available for Blender to use.   Select the "Open" button and choose the HDRI file you want to use using the "File Browser".

Now, when you choose Render, you will be using the lighting from the HDRI file.
...
When you Render your model from here on, this would be your background if you had not chosen "transparent" in the FILM option earlier. 

[TIP]
The `Abandoned Slipway` HDRI from https://hdrihaven.com comes recommended by *Josh Gambrell* for neutral outdoor lighting in the past. There is also a file called `hdri_004_nordicfxnet.hdr` aka "Railroad Crossing" from  http://www.nordicfx.net/  as it seemed slightly more appropriate.  I did scale it down from 4K to 2K though.

[TIP]
There is a free HDRI addon that makes setting up the World HDRI lighting image files a bit easier called "EASY HDRI". _Totally optional of course._
...
[NOTE]
 A lot of details have been snipped out for brevity. Regarding handy folders to create,  EXPORT is for renders and such, IMPORT is for things being converted from 3DCANVAS and TSM or other external 3D model sources. PROJECTS, should be obvious, but using prefixes and a naming standard to make things easy to find. I use  a SHARED folder for things that all projects will share, like Asset Libraries and common textures.  I prefix my Freight cars with FRT, Scenery with SCN and Engines with ENG... you get the idea.
...
The information presented below is not specifically about modeling with Blender, it is about how to create items that comply with the simulator requirements and guidelines.  Some references to old 3DCrafter notes are included here, as well.

(((Content Creation, "Basic Overview")))

[TIP]
Reminder, this document is basically a "notebook" and it started life as my collection of notes about content creation.  It may seem to jump around a lot while the concepts are being gathered.  Expect changes and updates.


=== Orientation
((( Content Creation, "Orientation")))

There is a need to properly orient your model if you are making rolling stock or an engine.  If you have used 3D modeling software in the past, maybe 3D Crafter / 3D Canvas or Maya, etc where you might be accustomed to Z axis being DEPTH and the X axis being WIDTH and Y axis being HEIGHT.  Blender orientation is similar to 3DS MAX where Z is HEIGHT, X is WIDTH and Y is DEPTH.

When working on items that roll on the track, the FRONT of a model is aimed towards the Positive values of the Y axis and the REAR of the model faces the negative Y axis. 
...
For the Blender Exporter to properly process your model, you must have a "Collection" named `MAIN` in your scene. This is the root of your model hierarchy. Other exported collections will be children of this collection, but `MAIN` will not contain your model directly. Models are placed in sub-collections for each LOD, such as `MAIN_700` and `MAIN_2000`. These LOD collections contain the actual parts of your model.
...
[TIP]
The way to get the correct layout of a dual 2 axle `BOGIE` `WHEEL` arrangement is to have all *wheels* use the center of their axle as the pivot point and the bogie use its default center of mass as its pivot point. In general, all other parts in a model will all use world origin as the pivot point.  

[NOTE]
The primary body part mesh itself does not need to be called MAIN. Modelers have been using that as a convention since 2002, so it has become a standard _thing_ to do, but it is not a requirement for {OR}. The Blender MSTS/Open Rails exporter requires `MAIN` as the name of the _COLLECTION_ that contains your model objects.

[WARNING]
Be careful with selecting all objects in object mode and then applying "all transformations", as it will reset all objects to have their origin (pivot point) to be `world origin`. This would change the pivot points of the bogies and wheels, etc.

=== Standard 2 Axle Freight Bogies
(((Content Creation, "Standard 2 Axle Freight Bogies")))
...
If an additional axle is needed, use WHEELS13 or WHEELS23 located behind the bogie pivot relative to forward motion and shift wheel set 2 to the center of the related BOGIE. It's is important that the naming sequence remains (11 to 23) as shown in the diagram or wheels will turn backward and shift improperly in {MSTS}. 
...
NOTE: These are primarily used for STEAM locomotives. Animating these wheels is not automatic and must be done using the animation tools within Blender. The steam wheel and linkage animation uses a series of 16 frames. In general, this is a non-trivial animation task and should be treated as an advanced topic.
...
Scenery objects, for example, would require an `sd` file if creating for {MSTS}, however, {OR} uses a different method to define scenery and it makes the SD file obsolete under many conditions.

Also, see <<creating-rolling-stock-and-locomotives,Creating Rolling Stock and Locomotives>>

=== Include Files in Configurations
(((Configuration, "Include")))

The use of `Include()` statements in SIMIS files is becoming more common in {OR} configurations, but the INCLUDE feature is entirely optional.


In both Microsoft Train Simulator (MSTS) and Open Rails Train Simulator (ORTS), `.wag` and `.eng` files are essential files that allow the simulators to identify an asset as either a locomotive or non motorized vehicle.  Historically, these files are large and complicated and they contain a large amount of replicated data among similar items.  Historically, MSTS files needed to be fully self-contained with limited ability to make callouts to other files.  In ORTS, the `.wag` and `.eng` files are much simpler and easier to manage since they now have the ability to `include` other ready-made `.inc` files into them, resulting in a smaller file size and easier management.

This is particularly beneficial when you are creating a group of similar cars that may only differ in a few details or maybe just the car reporting marks.

The `INC` file is an external file that doesn't even need to be in the same directory as the `.wag` or `.eng` file, although sometimes that option makes the most sense.  The `INC` file just needs to be defined in a way that {OR} is able to find it and include it into the `.wag` or `.eng` file.

[WARNING]
When `.INC` files are used with your `.ENG` and `.WAG` files, they need to contain a blank line at the beginning of the file.  For safety sake, you should also make sure that they have a blank line at the end of the file as well.

When ORTS loads a locomotive or car, it reads the beginning of the main `.eng` or `.wag` file, until it gets to the “Include” statement. It then searches for the `.inc` file in the specified file path, after which it reads the data from the `.inc` file, and inserts the `.inc` file's data into the locomotive or car's physics data. When ORTS reaches the end of the `.inc` file, it then returns to the main `.eng` or `.wag` file and continues reading its data until it either finds another “Include” statement (at which point the `.inc` file reading cycle starts again) or reaches the end of the main `.eng` or `.wag` file.


*For file locations, there are four general locations used* 

* In sub-folder in the .wag or .eng folder.
* Directly in the .wag or .eng folder. (Could get messy so use with caution)
* In a special folder somewhere else specific to `.wag` or `.eng` files having the same .s file.
* In a special folder somewhere else for use by many `.wag` or `.eng` files, referred to as `common` folders.

You should try to come up with your own naming standard that allows for common items as well as shared items specific to a particular master shape file.

==== General INC file structure

(((Configuration, "Include, Structure")))

INC files are still Standard {OR} configuration documents, so SIMIS format file rules and structure still apply.  But there are some additional considerations to keep in mind when creating INC files.

* The first line of the INC file must be left blank
* There should be no SIMIS header in the file, so no "SIMISA@@@@@@@@@@JINX0D0t______" line
* It is probably quite helpful to liberally use "COMMENT()" sections in your INC file
* INC files are only really useful for configuration items that are shared among multiple locomotives or cars.
* INC files are for {OR} only, as MSTS cannot use them.
* {OR} will read include files in a top down fashion.  Any duplicated content items would be overwritten by the last include file read.

==== Items well suited to INC files

* Coupler Details - Coupler details that are common to many locomotives or cars.
* Wagon Details - Wagon details that are common to many locomotives or cars, such as SMS (sound) and Empty or Loaded states vehicles.
* Brake Details - Brake details that are common to many locomotives or cars.
* Lights - Lights that are common to similar vehicles.
* Davis Coefficients - Davis coefficients that are common to similar vehicles.

Just be sure to make sensible choices as it can get complicated pretty quickly.

==== Specifying relative references


(((Configuration, "relative reference")))

Getting {OR} to locate your INC files is important.  You will use relative references to refer to include files by including in a way that references their location to the main file.  Here is an example of how to do this:

* if the INC file is in the same folder as the main file, use a relative reference like this: `INCLUDE("filename.inc")`
* if the INC file is in a subfolder, use a relative reference like this: `INCLUDE("subfolder/filename.inc")`
* if the INC file is in a different folder, use an absolute reference like this: `INCLUDE("C:/path/to/filename.inc") or use `referential naming` such as `..\common.inc\filename.inc` to refer to a file in a common folder that is up one level from the current folder location.


[NOTE]
There is a way to make content that uses this technique that remains compatible with legacy MSTS, but it will not be covered in this document.
...
=== Config File Deconstruction

(((Configuration, "Deconstruction")))

To create a working configuration file, we need to understand the different parts of the file and how they work together.  Here is a brief overview of the different parts of a configuration file:

* Header - This is the first line of the file and contains information about the file type and version.
* Body - This is the main part of the file and contains the actual configuration data.


==== Header

The first line of the file is the header and contains information about the file type and version.  The header is always the first line of the file and always starts with "SIMISA@@@@@@@@@@JINX0D0t______".  The header is followed by a blank line.  The header is not part of the configuration data and is used by the simulator to identify the file type and version.  Yes, differernt file type will have different header lines.
...
The body of the file contains the actual configuration data.  

Below is a randomly selected *basic*, MSTS-compatible body of a configuration file:

[source,text]
----
Wagon ( CP_218041_LD 
	Type ( Freight ) 
	WagonShape ( CP_218041.s )
	Name ( "HiCube 60 CP 218041 LD" ) 
	Size ( 3.108m 5.211m 20.531m ) 
	comment ( 38.46t empty, ~115t full ) 
	Mass ( 120t-us )
	WheelRadius ( 36in/2 ) 
	InertiaTensor ( 
		Box ( 3.1m 3.54m 19.931m ) 
	) 
	Comment("===================== Coupling =====================")
	Coupling ( 
		Type ( Automatic ) 
	...
	Commant("===================== Derail, Davis and Braking =====================")
	DerailRailHeight ( 5cm ) 
	DerailRailForce ( 2.5*115t ) 
	DerailBufferForce ( 1123kN ) 
	NumWheels ( 8 ) 
	ORTSBearingType ( Roller ) 
	ORTSDavis_A ( 1168.33 ) 
	ORTSDavis_B ( 18.9210 ) 
	ORTSDavis_C ( 1.928619 ) 
	Commant("===================== Brakes =====================")
	BrakeEquipmentType ( "Handbrake, Triple_valve, Auxilary_reservoir, Emergency_brake_reservoir" ) 
	BrakeSystemType ( "Air_single_pipe" ) 
	comment ( Load/Empty valve? = Yes ) 
	MaxBrakeForce ( 67.665kN ) 
	MaxHandbrakeForce ( 112.776kN ) 
...
Within this file you can see all of the key sections of the configuration file.  The first section is the Wagon definition, which contains information about the wagon itself, such as the type, shape, name, size, and mass.  The second section is the Coupling definition, which contains information about the coupling between the wagon and the train, such as the type of coupling, the stiffness, damping, break force, and velocity.  The third section is the Derail and Braking definition, which contains information about the brake system, such as the type of brake equipment, the type of brake system, and the brake cylinder pressure for the maximum brake force and Davis Coefficients.  The final section refers to the sound file used by the wagon.

You will notice that the S (shape definition) and the SOUND entry are references to external files.  These are references that are compatible with legacy MSTS.

[TIP]
The coupler and braking sections could easily be shared among multiple models, so they could be placed in an INC file and included in the main configuration file.  This would make the configuration file smaller and easier to manage.


=== Example ORTS Configuration Layout

(((Configuration, "Sample Layout")))

While the `Include()` statement can be added to any SIMIS file. AFAIK there isn't much use for them outside of `.wag` and `.eng` files. Here is an example of how the ORTS configuration files are laid out using the INCLUDE format:

We will outline  an example of a standard boxcar WAG configuration file.  Since my work is focused on RailSimStuff.com, I will use the RSS prefix for my files.

* We will use NAVS Freight Sounds - NAVSFR10.ZIP 
* We will use RSS_COMMON.INC - RSS_COMMON.INC - Coupler Details
* We will use RSS_WAGON.INC - RSS_WAGON.INC	- Wagon Details
* The main WAG file will contain the Freight ANIM decal definitions

image::images/folderlayout.png[]




[NOTE]
If you use Visual Studio Code, you can use the `File: Open Folder` command to open the folder containing your `.wag` and `.eng` files.  This will allow you to easily navigate to the `.inc` files and edit them.  There is an addon for VSCODE that understands syntax highlighting of ENG and WAG files, so you can see what is happening in real-time.  https://github.com/matejpacha/ORTSFileSupportForVSC
...
It is felt that these comments about creating content from Erick are so important that they need to be documented here as a reference. These details have been extracted from forum posts at Elvas Tower and are meant as a guide if you want to emulate his results.  While these are not hard rules, they definitely give us something to thin about and try to work with.  

[WARNING]
Erick supplies a lot of details about his approach to modeling here. It's worth reading and retaining these as guidelines for your own modeling. They are not hard rules, but they are a good guideline.
...
* Generally, it's wise to use whatever units match your reference materials to avoid unnecessary conversions. For example, when I build a Boeing, I work in decimal inches. When I build an Airbus, I work in decimal meters. For most US stock, reference materials will be in inches.  Its not a rule though.  If you have a scale calculator handy, its not hard to convert U.S. customary units  to Metric and visa-versa. (And you may learn to actually like the metric system.)
...
From Pete Willard:: One of my mentors while I was in my beginning stages of learning Blender was Josh of the Blender Brothers.  I subscribed to their Youtube channel and I get regular email updates about their ideas and helpful tips.
