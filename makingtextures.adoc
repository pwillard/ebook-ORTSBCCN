...
==== Can Blender Import DDS Files Directly?

(((Blender, "DDS")))

Yes, Blender can now import DDS files directly. Blender has built-in support for reading and using DDS textures. However, the quality of the import and how you use the textures depends on the specific DDS file and your project's needs. In modern releases, you don't need any additional add-ons to import DDS files in Blender. Just like importing any other image format. Refer to the COMPRESSION TYPE table above.
...
[TIP]
Ensure you are keeping uncompressed versions of the source files in the editing tool's native format that you can load for editing so you don't accumulate compression artifacts.

* With *GIMP*, you would use `export as` and then choose `select file type` and set the options for DDS such as Compression, mipmap, etc. Latest versions of GIMP seem to come with DDS support so no post-install plugin is needed.
...
* With Affinity tools, you just save your master file since saving to a "common format" like PNG is done using an EXPORT option.
...
(((Vector Graphics, "Font Tricks")))

==== Tweaking Fonts With Affinity

Lets say you want to create the Union Pacific distinctive outline lettering.  You  may have noticed a font available all over the web called "City Of" by Ben Coifman of RailFonts.com.  Ben released this lettering back when he was still in college and the Internet never forgets.  You can probably get a much improved version direct from their web site but that I leave up to you conscience regarding "supporting the little guys".

Anyway, you have the font and you have Affinity Studio or the older Affinity Designer installed.  So lets enter some text with the "CITYOF" font.  Make sure you are in VECTOR mode.

image::images/Screenshot 2026-04-08 125803.png[]

Change it to RED... 

image::images/Screenshot 2026-04-08 125855.png[]

So you get: 

image::images/Screenshot 2026-04-08 125927.png[]

The next step is to choose the VECTOR _TAB_; with the TEXT selected, choose "convert to curves" in the pulldown menu.

image::images/Screenshot 2026-04-08 130033.png[]


And finally, choose the outline thickness; here I chose a value of 7.8.  And it's done.

image::images/Screenshot 2026-04-08 130104.png[]
...
image::images/uvlayout1.png[]

Above you can see how the image was painted to match the UV layout that was exported from the 3D model. The texture in this case has already been created and updated in Blender with the painted image.
...
.Author Tips
----
Blender supports many third-party add-ons. This section only covers a few free tools that are useful for Open Rails work. Other modelers may prefer different add-ons, so experimentation is encouraged.
----

image::images/AO_Init.png[]

In Blender 4.2 and newer, many add-ons are managed through *Preferences > Get Extensions*. *Texel Density Checker* is available there, while *UV Layout* remains an official add-on under *Preferences > Add-ons*. *TexTools* is installed separately from its GitHub ZIP file.
...
Let's do a quick review of terms again...
...
Baking:: - refers to the process of capturing detailed lighting, shading, and other surface information, such as ambient occlusion and reflections, and storing it in a texture map. These effects are created with ray tracing and add a great deal of "pop" to any model.
...
PBR Materials:: - Physically Based Rendering (PBR) uses multiple texture maps to describe how a surface reacts to light. For traditional `.S` models, you may still rely on baked lighting and a single main texture. For glTF models, PBR maps can provide more realistic reflections, depth, and surface response.

glTF:: - A model source format supported by {OR} that can use PBR materials. The `.S` file format is still supported and is not going away.

[TIP]
Baking ambient occlusion layers requires objects to occupy unique texture real estate. If you want to apply details or baked textures, make sure no UV island overlaps another. If you have used the "Project From View" option while UV mapping, AO baking will be troublesome unless care is taken to deal with overlaps and hidden faces. Baking also has an option to adjust the margin size; keep the margin as small as practical while still preventing texture bleeding.
...
On the first two images, you can see the *Swatches* I use on various parts of the model; the hinges are probably the most identifiable. Multiple parts overlap in these small squares, so baked {AO} can make a mess if it is included there. In your paint program, place the *Swatch* layer above your {AO} layer to avoid the problem.

.Flat texture without baked ambient occlusion
image::images/Prime-Flat-No-AO.jpg[]

.Flat texture with baked ambient occlusion
image::images/Prime-Flat.jpg[]

.Finished texture with additional detail
...
I'll share a decal creation technique here using Python code for the so inclined in the programming section of this documentation.  The benefit of a code-based approach is that you have a nearly exact idea of how the texture is laid out and can easily adjust the layout to your needs.  The disadvantage is that you have to have the Python programming language installed on your computer along with required graphic packages the code depends on.  If you are not comfortable with Python, then you can use the manual method to lay out your decals, it's just a bit more time consuming.
...
=== Creating Alpha Channels with Affinity Studio

Affinity Studio, formerly Affinity Photo and Designers Suite, is a popular photo editing software that has a built-in alpha channel editor. It allows users to create alpha channels for transparency in images. You can use formats like PNG or TIFF that support alpha channels when working with images in Affinity applications.

To begin working with alpha channels in Affinity, select the PIXEL mode and choose the layer you want to work with in the Layers panel. You would then add a MASK layer by choosing the MASK tool at the bottom of the layers panel.  When you select the brush now, you will be able manipulate the ALPHA mask for the image using the brush options.  White foreground brush with remove alpha mask and the Black foreground brush will add alpha mask.

When you are done creating the mask, using the brush or selection tools, you can refine it using the "Pixel" mode "Refine Mask" to visualize and refine the edges of the mask using the available 'sliders'.  Apply to complete the "Refine Selection" option.
...
For texture pre-processing, particularly for photo-based texturing, use a perspective-correction tool when the photo was not taken square-on to the object. The older tool many content creators used for this was Shoebox: https://renderhjs.net/shoebox/. Shoebox still works for some users, but it requires the Adobe AIR runtime, which is now maintained by Harman rather than Adobe: https://airsdk.harman.com/download

For current work, try TextureRipper first. It provides a modern replacement for the texture-ripping workflow without depending on the abandoned Adobe AIR runtime.

image::images/shoebox1.jpg[]

The "texture ripping" option in Shoebox allows you to clip pieces from a master image and adjust the alignment while you do it. This is a useful way to get a good start on photo-based textures if you already have Shoebox installed.

image::images/radio1.png[]

The image to be manipulated should be in your copy/paste buffer before starting the texture-ripping process. You then click four adjustment points on the image, and the program adjusts the image to fit those four points.
...
Raycastly TextureRipper is a free, open-source tool for fixing perspective distortion in photos used for texture creation. It lets you extract usable textures from angled source photos so they become easier to turn into accurate textures for 3D models. Download it from its GitHub repository: https://github.com/raycastly/texture-ripper

TextureRipper was inspired by the Shoebox Texture Ripper, but it uses a newer interface and avoids the deprecated runtime dependency. If you are starting fresh, this is the better first choice.

image::images/matches-demo.jpg[]
...
AceIt is a program designed to produce Kuju ACE format texture files from many common image formats. It is intended to be used as a replacement for the {MSTS}-supplied tools `MAKEACE` and `MAKEACEwin` tools.
...
ACE2DDS went through a start-over release in version 1.2. The older Microsoft DirectX `tex2dds.exe` / `texconv` path has been replaced because it did not generate reliable DDS output for this workflow. Current PNG to DDS conversion is handled by the PowerShell `png2dds.ps1` script, which uses ImageMagick.

The download contains these main tools:

* `ace2png.exe` - This tool converts the ACE texture(s) file to a PNG file.
* `png2dds.ps1` - This PowerShell script converts PNG file(s) to DDS texture files using ImageMagick.
...
To use the `png2dds.ps1` tool, run it from PowerShell 7 or newer with the following command-line arguments:

```
png2dds.ps1 [[-Source] <path>] [[-Destination] <path>] [-Recursive]
```

`Source` can be a folder of PNG files, `Destination` defaults to a `dds` folder, and `-Recursive` processes PNG files in subfolders. The script checks the image channels with ImageMagick and selects DXT1 for images without alpha and DXT5 for images with alpha.

[TIP]
You can process entire folders of ACE texture files at once with `ace2png` if you supply a folder name as the source. For PNG to DDS conversion, run `png2dds.ps1` on a folder and let it write the converted files to the destination folder.
...
[NOTE]
Affinity products do not support exporting to DDS directly, but you can use Paint.NET, GIMP, or command-line tools to convert the file used by Blender into the DDS format usable with {OR}.

When you export your model to the `S` file format using the MSTS/{OR} exporter script, export the DDS texture from Paint.NET, GIMP, or a command-line converter as a follow-up step.

[TIP]
Remember that having fewer texture files for your model reduces _draw calls_ at run time. A larger texture uses more video card memory, but modern cards are designed for this.

The trick with Alpha channels in Blender is to remember that the hard work is done for you.   You don't need to put things like *window glass* in a separate object collection.

==== The Shader Editor

If you look at a typical setup in the Shader Editor for a texture that contains an alpha channel, you would see this.

image::images/alpha1.PNG[]

It looks straightforward, but heed this warning: this is related to what you `SEE`, not necessarily what you export.
...
For most scenarios where an alpha channel is used, this `Alpha Blended` setting is good enough and it works OK in my experience, so try it first. It is the grayscale alpha channel used for glass, while `Transparency ON/OFF` is the one-bit mask where only black RGB (0,0,0) is transparent in a mask.
...
When using the new glTF model source format, it is possible to make use of PBR materials.  By using PBR materials, we get a more dynamic result in how lighting and shadows work on a completed 3D model.  The result is added realism, making objects look less flat and unrealistic in certain situations.  The glTF format is an open standard supported by a multitude of existing products, unlike the proprietary `.S` and `.ACE` file formats inherited from {MSTS}.

You will now be able to modify the shine or dullness of a surface as well as roughness and smoothness on surfaces. You can even infer a certain level of depth by using a form of a height map.

The terms used to describe these additional image maps are:

1) Base Color - The primary texture, sometimes referred to as "Albedo" or "Diffuse"
2) Normal - The Height Map, sometimes referred to as the Bump Map.
3) Emissive - Light Emission
4) Roughness  - Wood Dull, Metal Shiny
5) Metallic - Metal or non metal amount
6) Ambient Occlusion - Shadows

[TIP]
PBR Materials still support Alpha Channels, don't worry.

=== PBR Workflow

If you decide to use 3rd party tools to help with your PBR image workflow, remember the alternative terms mentioned, (Albedo and Diffuse) as they are often used to refer to the Base Color of your texture.  There are some free PBR image generation tools as well as some PBR related add-ons for Blender.  These will be discussed later.

[NOTE]
If you have used Blender for a while, you have probably already seen the image adjustments for PBR in the "Principled BSDF" shader node as Metallic, Roughness, and Normal inputs.

Your first step is to generate the Base Color texture.  When using {MSTS} or traditional {OR} models, this is actually where you stop in texture generation.  With PBR textures, it's just the beginning.  Base Color is the name of the input used on the "Principled BSDF" shader node for this type of image.

A metallic map is next in the list and is used to simulate a metal surface, which often reacts with light in a specific way.  The Metallic Map is often a binary color image that is made of just black and white pixels (0 and 255) color values. The metallic setting in the shader can be adjusted from 0 to 1. Color Space is non-color for this image.

The Roughness Map shows how rough or smooth the surface is.  This affects the sharpness of reflected light, in essence, how much light is scattered by the surface to produce gloss or matte surfaces. The Roughness setting in the shader can be adjusted from 0 to 1. (0 = gloss 1 = matte) Color Space is non-color for this image

The next step in PBR texture generation is generating the Normal Map.  You might recall from other sections in this document that a Normal is related to the Facing Direction of a 3D surface. Whenever light hits this specific surface, the normal setting would be able to allow for the light reflection of the "outside" surface.  The NORMAL is also a determination of the inside and outside facing direction of a surface.  An improperly facing normal will cause incorrect lighting of a surface to occur.

So, how does the Normal PBR texture work?   The Normal Map is used to develop a sense of depth in a surface or in other words, the bumpiness. The RGB values in the image translate to XYZ coordinates without changing the underlying geometry. A normal map can be used where extra geometry in the underlying model would be costly. (Think: doors on the long hood of a diesel engine)   Normal Maps often look very purple, as a side effect of using the RGB color space for x,y,z displacement data.  Color Space is non-color for this image.

[TIP]
When using the Shader Editor in Blender with Normal Map images, you would feed the image node into a "Normal Map" node before feeding it to the "Principled BSDF" shader "Normal" input.



The Emission Map can be used to simulate a source of light.  It uses RGB data to perform this task. At this time, this is untested.


[TIP]
There is a potentially useful *paid* Blender Add-on available from SuperHive called *PBR PAINTER 3* that will assist with the PBR image workflow. There are also a series of tutorial videos on how to use *PBR Painter 3* available on Youtube. https://youtu.be/ntdlvndv1tE?list=PLMN9uGgVtpSt9hTaiDAKYJ1wIS1nQvTf9

[NOTE]
Additionally, there are online tools such as PBR FORGE that will assign with PBR image generation.  For offline installable software, there is a program called *Materialize* from BoundingBox Software.
...
.Daytime source texture
image::images/Image1.jpeg[]

.Night texture counterpart
image::images/Image2.jpeg[]

.Second daytime source texture
image::images/Image3.jpeg[]

.Second night texture counterpart
...
.Snow texture counterpart
image::images/Image5.jpeg[]

.Second daytime source texture
image::images/Image3.jpeg[]

.Second snow texture counterpart
...
=== Ambient Occlusion Steps - Summary
...
[TIP]
Success with {AO} is strongly related to how the underlying model is created and textured.  A surefire way to have issues is due to lazy texture mapping.  For example, if you relied on "Project From View" to generate the UV assignment on an object and did not MOVE the UV coords of the "back side" of the object, the generated AO shadow will be placed at the same location as the front side, ruining the effect. You can also correct this issue by removing the backside faces from the object if they will never actually be seen.
...
In the Render Properties tab I have set the samples to 2048, which is high but gives a fine high-resolution AO bake. Denoise is enabled because it improves the quality of the bake. I use the OptiX Denoiser because it is faster on my graphics card. Bake type is set to Ambient Occlusion. Device is set to GPU Compute because my Nvidia 2070 is considerably faster at baking than the CPU. Current Blender versions can use the Vulkan display backend, which may be faster than OpenGL on some systems. Test both to see which is faster for you using Edit > Preferences > System > Display Graphics > Backend > Vulkan or OpenGL.
...
I then export this from GIMP as the texture file that is applied to the model in Blender, in this model it is generic_SD40-2_4096.png and export it as generic_SD40-2_4096.dds for use directly in Open Rails.
...
When baking occlusion, here are some general tips adapted from Wayne Campbell's notes:
...
3. Coplanar surfaces are a problem, including two-sided surfaces; they may bake black.
4. Set 'Render Engine' to Cycles.
5. Set Render Max Samples low, such as 8, for testing. Use higher values such as 128 or 256 for better final quality.
6. If your model has secondary LODs, ensure they do not cast a shadow on your primary model. Use 'Disable In Render' in the outliner.
...
* In the Bake tab, set the Bake Mode to Ambient Occlusion.
