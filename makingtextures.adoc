...
.Author Tips
----
Blender is an application that can utilize 3rd-party addons and there are thousands of them.  I'll only be discussing a few that I find essential for Open Rails related work. These are free. I've tried many others as well and another modeler might prefer a different set of addons. Experimentation is key here. 
----

image::images/AO_Init.png[]

In this setup, Blender 4.5 is being used and since Blender 4.2 many of the external addons are now available in the *Get Extensions* option under *Preferences*. *Texel Density Checker* is available via *Get Extensions* in Blender 4.3. *UV Layout* is an official Blender addon available via *Add-ons* option. The last one,  *TexTools*,  is available from GitHub and needs to be installed from the downloaded zip file.
