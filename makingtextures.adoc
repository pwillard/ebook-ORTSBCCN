...
[TIP]
Baking ambient occlusion layers require objects to occupy unique texture real estate - if you want to apply details or baked textures - you need to ensure no UV Island overlaps another. If you have used the "project from view" option while UV Mapping, AO Baking will be troublesome unless care is taken to deal with overlaps and hidden faces.  Baking also has an option to adjust the margin size and it is probably best to keep the margin to a minimum.
...
=== Alternative to Shoebox - TextureRipper

(((TextureRipper, Tools)))  

TextureRipper is a tool that can be used to fix perspective distortion in photos for texture creation. It is a free and open-source software that allows you to easily adjust the perspective of your images, making it easier to create accurate textures for your 3D models. You can download TextureRipper from its GitHub repository: https://github.com/raycastly/texture-ripper
...
This tool is based on the Shoebox Texture Ripper. The author didn't like the controls on that tool and the fact that it was relying on deprecated software. So they decided to build their own version.
