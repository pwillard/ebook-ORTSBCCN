...
[WARNING]
'DDS' files use 'lossy' compression which means that you will lose detail and color information compared to your original texture.  You should treat a DDS file like a PDF... it's a final document that is not designed for future editing.
...
*  You can't just use ANY bitmap editor to edit files... you need one that actually understands files (like TGA) with an alpha channel embedded.
...
When you finally export your model(s) to S file format using the MSTS Exporter script, export the DDS format from Paint.net or GIMP.
