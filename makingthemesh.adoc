...
[NOTE]
Since this is primarily a set of preset shapes with customizable values and locations, Blender's scripting language could be used to recreate a similar workflow in Python. A tool like this would be a useful companion to the maintained Blender MSTS/Open Rails `.S` file exporter.
...
The exporter documentation is maintained with the project on GitHub: https://github.com/pwillard/Blender_MSTS_ORTS_Exporter
...
[NOTE]
Pete Willard's ACE2DDS tools are still under development. The project can batch-convert ACE files to PNG files, and the version 1.2 start-over release changed the PNG to DDS workflow. The older Microsoft DirectX `tex2dds.exe` / `texconv` path is no longer used because of problems with the DDS files it generated. The current `png2dds.ps1` PowerShell script relies on ImageMagick to generate DDS output from PNG sources. You can find the tools at: https://github.com/pwillard/Ace2DDS
...
Everyone has a way of working creative projects that suits their comfort level. The sequence of steps you become comfortable with becomes your workflow. This is based on the tools you use, the processes you trust, and any scripts you write or acquire to help with your projects.

As a quick discussion of workflow, the author will share their workflow as a reference.

Once you start a project, it will go through many revisions as you make progress on it. Save a fresh numbered copy after milestones are reached or when decisions are made about how best to proceed. You can fall back to older revisions if the chosen fork does not work out.

Once the model is underway, the initial stages of texturing begin. One way to start is to generate a blank PNG file at the size you need in the UV Editing tab, then save it. In the Materials tab, create a new BSDF material with an Image Texture node and use that newly created PNG file for the texture. It will still be default black. Assign that material to the model without worrying about the current UV mapping yet. Back in the UV Editing tab, map your UVs using the method that makes the most sense for the object. With the mapping done, use Export UV Layout to create an outline `.png` file that you can import into a program like Affinity. From there, you can map proper texture imagery to the UV map outlines you exported.

[NOTE]
With Affinity, when importing the UV map layout `.png` file, use the Place option and paste anywhere on the screen. Then, in the Transform panel, set the location to X=0 and Y=0. The placement will be aligned correctly.

Create an export folder where you assemble all of the required files for {OR}. This is where you export the `.S` file from Blender, and it is also where you place the `.DDS` files, `.wag` or `.eng` files, and any other items needed by {OR} to make the content usable.

Since you are likely using PNG files for textures inside Blender, you need a conversion process from PNG to ACE or PNG to DDS. DDS is recommended because the conversion is simpler and is not based on a proprietary format. A script was developed to handle this process for a folder or an individual file. The script can be found in the `SCRIPTS` folder of the GitHub project.

In the final stages of the project, generate the Ambient Occlusion map. Refer to the ambient occlusion section for details. When the AO bake image has been generated, place the AO layer into Affinity at location 0,0. Put the layer at the top of the stack, set opacity to around 30%, and change the layer mode from "Normal" to "Multiply".

With this bake layer added, export to PNG again, convert to DDS, and place the result in the export folder. Test the content at this point using the TSRE shape viewer option.
...
==== Rolling Stock: Importing Model Objects
