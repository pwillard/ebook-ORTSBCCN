...
The 3D Canvas/Crafter platform includes a train content ENGINEER add-on — Trainworks -> Train Simulator Engineer — that functions as a quick-start utility for assembling train models. It automatically places fully modeled wheels, axles, truck assemblies, and a base body element, all of which can be customized. Unfortunately, Blender lacks anything comparable to this feature. In practice, though, the time savings are modest; if you already have a prepared truck set and couplers, positioning them correctly is straightforward. That said, a similar add-on for Blender is certainly under consideration.
...
[TIP]
Version 4.4 and newer of the MSTS EXPORTER includes a button to define the textures used in the `.S` file as `.DDS` instead of `.ACE`. Though, if {or} does not find an `ACE` file, it will attempt to use a `DDS` file of the same name.
