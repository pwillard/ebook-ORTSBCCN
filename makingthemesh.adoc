...
[NOTE]
While you _can_ have Blender work in Imperial units (feet and inches), it may be far simpler to always work in the metric system since you won't need to change the "UNIT" settings in Blender.  You just might need to have a unit converter handy, and the easiest way is to just ask Google.

=== Some Basic Things About Modeling
(((Blender, "Modeling Tips")))

Some additional guidelines from Erick Cantu of NAVS
...
[NOTE]
====
These comments from Erick should be considered guidelines rather than strict rules. 
Many creators produce acceptable results while choosing not to follow them. 
However, the recommendations are intended to help maintain good performance in 
Open Rails, particularly when running long and complex trains on older hardware 
or lower-end graphics cards.

Items 1 and 2 should already be familiar concepts dating back to the early 
Microsoft Train Simulator era. Item 3 is especially important, as the temptation 
to create highly detailed objects should be balanced against what will actually 
be visible during normal Open Rails operation.
====

(((Blender, "High to Low Poly Baking")))

[TIP]
====
Additional details can be modeled directly onto the main mesh when the goal is
to generate a master texture. This technique allows extra visual detail to be
added without increasing the polygon count of the final model.

This process is commonly known as *hi-poly to low-poly baking*. A highly detailed
version of the model is created, but it is not used as the final in-game object.
Instead, the additional geometry is baked into textures that can represent
features such as nuts, bolts, rivets, doors, and other surface details.
====

=== Polygon Count and Vertex Density
(((Blender, "Polygon Count and Drawcalls")))
...
(((Blender, "Setting up to create a model")))
...
When inserting background reference drawings into Blender, it is not critical that you match up the image size to equal world space dimensions, but it can save a lot of re-scaling steps later. One of the main issues with using backgrounds in Blender is that some methods you might be accustomed to with other modeling tools don't work in Blender, such as applying a background image to a cube and using that as your reference, since in Blender, you only see the texture in a texture viewing mode. It will disappear in Xray/Wireframe and even solid shading modes.

When you insert a background using Blender's menu:Add[Image > Background],  the image WILL show up while you are in wire-frame modes... but note that by default the image will ONLY be visible from the axis it faces. Be careful when you install a background image as you need to be in the Axis View you want, since the added image will orient itself to your current view.  If you want to change this, you can enable the `Perspective View` check-box by clicking on the `EMPTY` and adjusting the setting in its properties window.
...
When using background images and modeling a freight car, for example, you would first figure out the distance between truck centers in meters and match the background drawing scale using the grid in the 3D view. Adjust the background image scale using kbd:[S] and position it using kbd:[G].

[NOTE]
When scaling with the mouse, the cursor position changes how sensitive the movement feels. If the cursor is close to the object when you press kbd:[S], small mouse movements can change the scale quickly. If the cursor is farther away, the same mouse movement changes the scale more slowly, making it easier to fine tune.



[TIP]
====
When working with images in Blender, you can install the add-on
*Insert Images as Planes*. After enabling it, a new option will appear
in the *Add* menu.

This tool creates a plane and automatically assigns the image as the
material texture. It also supports features such as alpha masking and
responds to scene lighting.

However, the image plane is a real mesh object with a material. It is useful
when you want an image in the scene, but a background image empty is usually
better for blueprint-style modeling references.
====
...
We will digress for a moment to share one of the reasons why using 3D CANVAS was considered helpful in the initial stages of making a model.

The 3D Canvas/Crafter platform included a train-content ENGINEER add-on — Trainworks -> Train Simulator Engineer — that functioned as a quick-start utility for assembling train models. It automatically placed wheels, axles, truck assemblies, and a base body element, all of which could be customized. Blender does not include an equivalent tool by default. In practice, though, the time savings are modest if you already have prepared truck sets and couplers; positioning those parts correctly is straightforward.
...
[NOTE]
Since this is primarily a set of preset shapes with customizable values and locations, Blender's scripting language could be used to recreate a similar workflow in Python. A tool like this would be a useful companion to Wayne Campbell's existing `.S` file exporter.

image::images/Result.PNG[]

Using this plug-in with 3D Canvas gave you a number of starting parts before modeling really began. In Blender, however, you normally create or import these parts yourself.


[TIP]
====
3D Canvas had a "render 2-side faces" option. Blender's current workflow is different: a plane may appear visible from both sides in Blender, but export formats and simulators may still treat it as a single-sided face depending on material and culling settings.

For reliable Open Rails content, model visible thickness instead of relying on a two-sided display option:

Create a new plane with menu:Add[Mesh > Plane], then add a Solidify modifier to give the plane real thickness:

1) Go to the "Modifiers" tab in the Properties panel.

2) Click the "Add Modifier" button and choose "Solidify".

3) Adjust the thickness in the Solidify settings.

If you want to make the thickness permanent, click *Apply* on the Solidify modifier.

[WARNING]
Solidify adds geometry. That is usually better for export than relying on viewport-only double-sided display, but it also adds faces you must UV-map and manage.
...
A good initial project is to create a simple scenery object by modifying the default cube.

==== Do we need a quick 3D Modeling primer?
(((Blender, "Modeling Primer")))

A fundamental concept in Blender is the close interaction between mouse and keyboard. The right hand is typically used for mouse navigation, while the left hand handles keyboard shortcuts. Although Blender has introduced optional pie menus and on-screen widgets in recent releases, it remains largely a key-driven program. Many operations can still be executed more efficiently through shortcut keys combined with precise mouse actions.

Like most 3D software, Blender is built around a set of core elements known as primitives. These are the foundational building blocks for creating models. Among them, the most commonly used for hard-surface modeling are the cube, followed by the cylinder and plane. Blender refers to these items collectively as mesh objects.

By adjusting the scale, rotation, and position of these primitives—or by manipulating their faces, vertices, and edges—it becomes possible to construct and refine virtually any shape. The process can feel overwhelming at first, but proficiency comes through hands-on practice. Observation and tutorials are helpful, yet true understanding develops only through active experimentation within Blender itself.

[NOTE]
====
It is important to watch and read tutorials to become familiar with Blender's features and workflow. However, practical experience is essential. Experimenting and creating a few simple models is the best way to build confidence and skill. 

For example, an excellent beginner project might be something straightforward, such as a 12-foot Jersey-style concrete barrier.  It is simple, structured, and easy to refine.
====
...
[TIP]
When starting a new project, approach it with the understanding that some or all of the work may need to be redone. As new and more efficient techniques are discovered, revising or discarding earlier attempts is a natural part of the learning and creative process.


Although Blender has a steep learning curve, even a beginner can produce a simple model like the one shown above by applying a few basic commands.

=== The 3D Viewport
(((Blender, "Viewport")))

The 3D Viewport is the main workspace in Blender. It is where you will see your models, textures, and other objects. It is divided into two sections: the 3D View and the Properties panel.

Adding a new MESH object to the scene can be done by clicking on the "Add" menu and selecting "Mesh" and then "Cube". This will create a new cube object with default dimensions of 1x1x1. You can adjust the dimensions by clicking on the cube and dragging the edges or corners.
...
[TIP]
Don't get too attached to the default layout.  It seems that the Blender developers like to change things up from time to time. Some of the examples shown here may not match the latest version of Blender.    
...
[NOTE]
Use of Blender's Pie Menus is totally optional. Some people find them to be a time saving device, others disagree.
...
.Box Select icon
image::images/Arrow.JPG[align="center"]
...
[NOTE]
There are three sub-modes in `EDIT MODE`; kbd:[1] Vertex Edit, kbd:[2] Edge Edit and kbd:[3] Face Edit. 

<<<

=== The four major edit-mode tools you are likely to use the most are listed below.
...
[TIP]
There are multiple bevel options in Blender version 5.  Worth looking into if thats the version you are using.
...
You can use the mouse to SLIDE the selected edge loop(s) into position before confirming. You can add multiple edge loops at once by using the scroll wheel on the mouse or by entering a value with the keyboard before confirming.
...
* kbd:[Shift + D] to duplicate the current object, then choose menu:Object[Mirror > Global Y]
* And finally, flip it. kbd:[S X -1] which translates to `SCALE`, in the `X` axis, amount `-1`, which will FLIP the object in the X-AXIS.

[NOTE]
You might be tempted to try to use the Mirror Modifier, however, that operation will ultimately LINK the objects and the option within the modifier that says "FLIP X" actually does not do what you might think.  The FLIP option here changes the active side of the Mirror.
...
* You can change the viewport shading options from studio to matcap and then select a lighting setup option to get different shading effects on your work.  Some options really do make it easier to see what you are working on, such as "Clay" or "Metal Car Paint". 
* When you create a new object from a primitive, it will retain that name, IE; CUBE, so it is a good practice to name the item as soon as you create it, otherwise you can end up with dozens of CUBE items in your collection, making it harder to find what you are searching for.
...
.Exporter
****
You will want to download and install the "MSTS Exporter". https://github.com/pwillard/Blender_MSTS_ORTS_Exporter/releases

The Documentation is included in the ZIP file.  The documentation is also available separately at this location:https://github.com/pwillard/Blender_MSTS_ORTS_Exporter
...
.Helper
****
There is also a "Work In Progress" helper script located at https://github.com/pwillard/Blender_trainsimstools   This add-on can generate the required Exporter Collection layout for you.
****
 ...
 * We will re-make my first Blender project;  The Jersey Barrier, 12', by 42" by 32".
...
[NOTE]
When you apply a modifier, you lose the ability to adjust it further.  Prior to applying it, you can still make adjustments to the shape.  In our case, we were done making a symmetrical object, so it was `OK` to apply the modifier.
...
* In the `EDIT MODE` window select  the `UV` tab , or press kbd:[U] and then choose `UNWRAP`.
...
* Under `BASE COLOR` you now see `+NEW` and `OPEN` icons.  Click `OPEN` and choose your existing texture file. The `UV SQUARE` background image in the UV Window should now display your image behind your `UV ISLANDS`.  If you choose `NEW`, you will have a blank image assigned to the material, which in this case is not helpful.

[NOTE]
The GITHUB page for this document has a DEMO1.ZIP file that contains the concrete texture I used.

(((Blender, "Unwrapping a mesh for texturing")))
...
* Choose menu:FILE[EXPORT > OPENRAILS/MSTS(s)] file, and choose where to save. Use the browser to select your project's `\final\` folder and modify the filename as needed...  I chose `barrier.s` for example.  The bottom of the screen will say "Finished OK" when done.
...
[TIP]
Version 4.4 and newer of the MSTS/Open Rails exporter includes a button to define the textures used in the `.S` file as `.DDS` instead of `.ACE`. Also, if {OR} does not find an `ACE` file, it will attempt to use a `DDS` file of the same name.
...
.ABOUT DDS SUPPORT 
****
With a layered master texture file, such as a `PSD`, you can keep the editable source file intact and export separate runtime textures from it. For Blender, export a `PNG` or `TGA` copy for material setup. For {OR}, export a `DDS` copy for the final simulator texture.

Also note that even if an S file references a `.ace`, Open Rails will attempt to find a `.dds` with the same name, and if found, it will use the `.dds` instead of the `.ace` file.  This is a nice feature. To me, relying on it does seem to be a bit lazy so it's best to always specify what your intended format is.
...
The final step is to copy the contents of the project's `FINAL` folder into the {OR} `trains` content folder for testing.
...
[TIP]
You can use the DECIMATE operation in Blender to quickly reduce the polycount of a shape while maintaining most of the topology of the original shape.  This can give you a head start on reducing what gets drawn on each LOD level in addition to removing the finer details that won't be seen.   Remember, though, reducing draw calls is usually more important than reducing polygons.
...
[NOTE]
Pete Willard's ACE2DDS tools are still under development. The project can batch-convert ACE files to PNG files, but the older `tex2dds.exe` path is no longer being maintained because of problems with the DDS files it generated. A newer tool, `png2dds`, has been created instead. It relies on ImageMagick to generate the final DDS output files from PNG sources. You can find the tools at: https://github.com/pwillard/Ace2DDS

==== Next Steps

Now that we have covered the basics of creating a model in Blender, we can move on to preparing the solution for use within {OR}.

1. Export your model as an `.S` file using the MSTS exporter.

2. Create all the textures and related files for your model. This means converting your textures to the proper format for {OR}: DDS or ACE. For DDS, you can use PAINT.NET to save the PNG files as DDS
...
* The .ace files (textures) go in the textures folder. Open Rails can also use .dds textures and they go in the textures folder.
...
Higher resolution models should have more LOD levels than lower resolution models.  Building the LODS is best done just before the model is exported and before separate pieces of the model are merged together, allowing you to isolate sections that will still be visible at a distance.  You really should save object merges until the very end.

Surely you can export test versions of the model before any merging of objects to see if the model is working as expected.   The goal in the end though is to simplify the final hierarchy in the model.

[TIP]
While Blender does have a method of saving incremental edits and numbered saves, you will probably be better served if you make use of the "SAVE AS" option and create your own WIP VERSION saves to numbered files... (IE; `ProjectA_01.blend`, `ProjectA_02.blend`, etc). When my project working folder gets crowded, I usually move all of the opder saves into a folder named "OLDER".

Initially, you will save the main model as the top Level of Detail version or LOD0.  You would then Clone this into a new collection, following the LOD NAMING convention of the MSTS/OR Exporter.  You would then hide the contents of the LOD0 collection and work on the cloned version, renaming the parts to match the new LOD LEVEL.  You would then start removing or simplifying objects that would not be seen at the new LOD distance.  IE; Interiors that won't be seen are a good candidate at this stage.  Gladhands, stirrups and any indide facing polygons are candidates as well.

On a 3rd LOD setting, you might want to just remake shapes from cube and cylinder primitives to keep the polycount low.
...
Everyone has a way of working creative projects that suits their comfort level.  The sequence of steps that you become comfortable with that you add to your repertoire of knowledge, or in other words, your workflow.  This is based on the tools that you become comfortable with using or maybe even processes a scripts that you write or acquire for yourself to help with your projects.

As a quick discussion of workflow, the author will share their workflow as a reference.

Once you start a project, it will go through many revisions as you make progress on it.  Save a fresh numbered copy after milestones are reached or when decisions are made on how best to proceed.  You can fall back on older revisions of the chosen fork doesn't work out.

Once the model is underway, the initial stages of texturing begin.  A way to start this process is to generate a blank PNG file of the size you need in the UV Editing tab, make sure to save it. Then in Materials tab, create a new BSDF material - Image texture and  use that newly created PNG file for the texture.  It will still be default black.  Now assign that material to the model, not caring about the current UV mapping fore the moment. Back in the UV Editing tab, you would map your UVs in the item using the method that makes the most sense for the object.  With the mapping now done on that object or objects, you would use the EXPORT UV MAP to get an outline layout `.png` file that you can import into a program like "Affinity Studio".  From here, you can map proper texture imagery to the UV Map outlines you exported.

[NOTE]
With AFFINITY, when importing the UVMAP layout `.png` file, use the PLACE option and paste anywhere on the screen, then in the "transform" panel, set the location to coordinate X=0, y=0 and placement will be perfect.

Create an export folder where you assemble all of the required bits to migrate the project to {OR}. This is a folder where you export the `.S` file from Blender, but it is also where you push the `.DDS` files and `.wag` or `.eng` files and any other items needed by {OR} to make the content usable.

Since within Blender you are likely using PNG files for textures, you will need to develop a conversion process for PNG to ACE or PNG to DDS.  DDS is recommended, since the conversion is a bit simpler and not based on a proprietary format.  A script was developed that handles this process on a folder or individual file basis, making this process rather easy. The script can be found in the `SCRIPTS` folder of the GITHUB page.

In the final stages of the project, you will want to generate the Ambient Occlusion map.  Refer to the ambient occlusion section for details.  When the AO Bake image has been generated, `place` the AO layer into AFFINITY, at location 0,0, and set the layer at the top of the stack and change opacity to around 30% and  change layer mode from "normal" to "multiply".  

With this bake layer added, export once again to PNG and convert to DDS and place in the EXPORT folder.  Test the content at this point using the TSRE "shapeviewer" option.

Look for mistakes or mapping issues and repeat the process until pleased with the results.

[TIP]
The TSRE shapeviewer can read `WAG` files, so if your model relies on FreightAnims(), you will be able to see them by specifying the `WAG` file to the viewer.
...
[TIP]
Blender 3.0 and newer include the Asset Library / Asset Browser workflow, which is a better way to store and reuse pre-made objects, textures, bogies, couplers, and other common parts across projects. If you are using a modern version of Blender, prefer the Asset Library workflow instead of the older manual library method described below.
...
[NOTE]
This is an optional legacy workaround for older Blender versions that do not have Asset Library support. It is not the recommended workflow for modern Blender versions. If your version of Blender includes the Asset Browser, use that instead.

A simple way to create a collection of reusable parts in older Blender versions is to *COPY* and *PASTE* specific parts from your current file into a separate library `.blend` file. This method requires two copies of Blender running at the same time.
...
* Choose menu:FILE[DATA PREVIEWS > BATCH GENERATE PREVIEWS] and choose the file name you used in the prior steps. Example: `Library_Freight.blend`
...
==== Saving in alternative export formats

You can also export selected parts as FBX or Collada (`.dae`) files. These formats can be imported later with menu:File[Import], and they can also be opened in other applications. The main drawback is that they are no longer native `.blend` files, so some object, material, or texture data may be changed or lost when imported back into Blender.
...
*  When your source objects are in an Autodesk `3DS` file format, you need to account for Blender version differences. Some Blender versions dropped direct 3DS support, while later versions restored it. If your Blender version cannot import 3DS directly, use Blender 2.79, another converter, or a current Blender version that supports 3DS import.

[TIP]
You can also run multiple versions of Blender side by side. If you need Blender 2.79 for a legacy conversion, import the `.3DS` file there, perform a `Select-All`, `COPY`, and then `PASTE` into your current Blender session.

[NOTE]
Blender {older} and newer versions once again support importing 3DS files.

*  UVMapping might survive an import operation... but material and texture selection probably didn't. You most likely need to manually assign materials and textures to your imported objects.

* The import step has also probably has assigned a new material names for each object and to be honest, it's easier to manage if you have all objects that use the same texture also use the same material (unless you need special alpha or reflective properties for something like metal or glass). 

* You also will probably need to manually tell the `S` file exporter (under MSTS Materials) where to find the texture file under the `BaseColorFilePath` input field in addition to assigning the texture to the material.

* Lastly, from personal experience, if you are importing a `DAE` (Collada format) file from say `3DCanvas`, you might encounter an issue with `UVMAP` assignment in `SHADER EDITOR` where you are not getting the right `UV map` assigned and you see no texture in the `Material Preview` mode  (Commonly called `LOOKDEV` mode).  

This is the second tab from the right in the viewport selections, by the way. Everything will LOOK right in the `Material Preview`, but exports will fail with a "Missing UVMAP in:" error when you try to use the `S` file exporter.  _See the next section for how to deal with this problem._
...
This code is also included in the add-on: https://github.com/pwillard/Blender_trainsimstools
...
.Example 40 x 80 x 20 background reference
image::images/40x80x20.jpg[]

This reference is low-resolution, but it is adequate for blocking out the overall proportions.
...
[NOTE]
By default in modern Blender versions, you may only see side or front background images when using the matching orthographic view. The images may disappear when you pan around your model. This behavior is controlled in the Data Properties of the background image object.
...
* Switch to front view kbd:[KEYPAD 1] and select `ADD->IMAGE->BACKGROUND` and choose the image from `twostall\reference` we saved there earlier.
...
* Use `BOX` select to choose the central vertices and shift them upward to match the roof angle, kbd:[G] kbd:[Z]
...
.Extruded roof faces before flattening the bottom vertices
image::images/ex2_8.JPG[]

The temporary over-extension is subtle in the screenshot; focus on the lower edge before it is flattened in the next step.
...
* Choose X-RAY mode by clicking the X-RAY button on the top right or kbd:[ALT] kbd:[Z]
* Press kbd:[3] to use face selection mode and choose the bottom faces.
...
* Choose `ADD MODIFIER` and select Boolean
...
[NOTE]
The completed version of the model can be found in the Trainsim.com file download section. Title: 2-Stall Reparations Depot
...
[TIP]
=== Example: 100-Ton Truck Model

A usable Blender example of a 100-ton freight truck is provided, based on Erick's original model. Available resources include the following:

* *Source files for NAVS N-Series freight car trucks*  
* *Source files for NAVS Z-Series freight car trucks*  
* *100T Roller Bearing Truck Blender Object*

Some of these items are also available through *Trainsim.com*.

Erick's truck models include attached couplers by default. In practice, couplers can be separated if desired, depending on the needs of the project. Keeping them attached allows the couplers to follow truck rotations accurately, which is the reasoning behind Erick's design choice.
