...
[IMPORTANT]
You will want to download and unzip the "Blender 2.8 to MSTS Exporter" from Elvas Tower and then read the instructions on how to install and use it. It includes very fine PDF file with instructions on how to use it.
...
* Now switch to the `SHADING` window using the top menu bar.  Note that the `NODES` window now has a `UVMap` node and the text field says "UVMap".  This is needed for the MSTS `S` file export.
...
* The exporter will ONLY create the `.S` file and assume your texture is `.ace`.  Converting your `.JPG` or `.PNG` textures to `.ACE` (or .`DDS` ) remains for you do as you will need to do this manually ourside of Blender.
...
[TIP]
Since I edited my copy of the MSTS/ORTS export ADD-ON to have an export as `.DDS` option as weall as `.ACE`, my `.S` file has the following difference in it
...
Due to this one change, I can use my current master texture file, which I edit in `PSD` format using PAINT.NET to keep the layers intact, and just save it from PAINT.NET as a `.DDS` file natively.   As a result, I do a minimal amount of after-the-fact editing.  To work in Blender, I save a copy the master `PSD` file as a `PNG` file and to work with {or} I save a copy of the master `PSD` file as a `DDS` file.

The next step would be to copy the contents of the project's `FINAL` folder into my {or} `trains` content folder for testing.  I have eliminated the need to edit the `S` file or do a texture file conversion (skipping a step where I would have had to use a tool like MakeAce) by using the supported DDS format files.

====


[IMPORTANT]
The Exporter script  will *only* create the model `.S` file and by default it assumes you are creating .`ACE` files. You still need to create the ENG or WAG or similar related files that define your model to the simulator.


=== Creating Level Of Detail Distance levels

Once you have your main model created, you should consider creating LOD entries.  These are easy to add or change by editing LOD collection name, eg create MAIN_0500 or rename MAIN_0700 to MAIN_0500. You add content to a new LOD level collections by dragging parts around or using the M key to assign collections. With collections, all your LOD assignments show in the outline panel and you can hide or un-hide whole collections as needed since your LOD assignments are also clearly shown in the object’s 'Collection' panel.  Making use of the filter options and checkboxes of the Outliner helps to focus your work.
...
I like to start constructing vehicles starting with the trucks, and for this I’m going to recommend downloading a *.blend file to simplify this.   Erick Cantu has graciously provided tricks that we can include in our own models.  They can be downloaded from Elvas Tower.
...
Using these existing objects, we can Append the ready made model into our initial blend file.
...
In realtime computer graphics, a technique called "level of detail" is commonly used in various applications ranging from commercial simulators to PC and console games.
...
Referring to some example snippets from the Outliner window in a Blender Working session...
...
Lets Look at some other issues here whith what is shown above... Don't forget capitalization. For Example: Bogie1 should be BOGIE1.
...
[TIP]
Don't forget to check out the sample locomotive file that Wayne provided with the MSTS exporter download.
...
== Baking Abient Occlusion

When Baking Ambient Occlusion, Here are some tips:
